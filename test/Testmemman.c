...
//next fit successfully finds empty slot from where it left off
void test_next_fit_S() {
    void* p = allocate(100);
    void* q = allocate(200);
    void* r = allocate(300);
	void* x = allocate(400);
	...
	header_t* hy = get_header_of_ptr(y);

	void* a = allocate(300); //should allocate in r's block
	void* b = allocate(100); //should allocate in y's block because of next fit

	TEST_ASSERT_TRUE(block_is_free(hp));  //p should still free

	TEST_ASSERT_FALSE(block_is_free(hr));  //blocks of r and y should be allocated
	TEST_ASSERT_FALSE(block_is_free(hy));

	void* c = allocate(100);

	TEST_ASSERT_FALSE(block_is_free(hp));
	//heap reset - no need, we allocated again with a b c the free'd blocks
}

//next fit fails and heap extends
void test_next_fit_F() {
    void* p = allocate(100);
    void* q = allocate(200);
    void* r = allocate(300);
	free(p);
...
int main(void)
{
	...
	RUN_TEST(test_next_fit_S);
	RUN_TEST(test_next_fit_F);
	RUN_TEST(test_split);
